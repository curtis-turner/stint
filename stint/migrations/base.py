...
from stint.migrations.exceptions import MigrationConflictError, MigrationGraphError
...
class RevisionGraph:
    ...
    def chain_from(self, current: str | None) -> list[Migration]:
        ...
        if len(heads) > 1:
            ...
            raise MigrationGraphError(
                f"multiple heads exist: {head_revs}. Run 'stint revision --merge <head1> <head2> -m ...' to merge."
            )
    ...
    def all_ordered(self) -> list[Migration]:
        """All migrations from base → head, for `stint history`. Assumes single head."""
