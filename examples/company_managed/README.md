...
migration-based path. It uses the schema in [`platform.py`](platform.py):
one project (`PLAT`), a `Bug` issue type, a `Severity` select field, screens,
and the schemes that bind them.
...
```bash
uv run stint validate --schema examples/company_managed/platform.py
```

Expect `OK: examples/company_managed/platform.py is valid.` This runs every
schema-level check without touching Jira.
...
```bash
mkdir -p .stint
cp examples/company_managed/devel.env.example.yaml .stint/devel.yaml
```
...
```bash
uv run stint stamp \
    --schema examples/company_managed/platform.py \
    --state examples/company_managed/devel.state \
    --env devel
```
...
uv run stint revision --autogenerate \
    --schema examples/company_managed/platform.py \
    --state examples/company_managed/devel.state \
    --migrations-dir examples/company_managed/migrations \
    --env devel \
...
```bash
ls examples/company_managed/migrations
```
...
Each op is idempotent and records its Jira id in
`examples/company_managed/devel.state`. A failed run is safe to re-run; it
resumes where it stopped.

## 9. Inspect

```bash
uv run stint current --state examples/company_managed/devel.state
uv run stint history --migrations-dir examples/company_managed/migrations
```
...
`examples/company_managed/migrations/` and `examples/company_managed/*.state`
are gitignored scratch. Delete them to start over:

```bash
rm -rf examples/company_managed/migrations examples/company_managed/devel.state
```
